...
class Helper
{
     ...
     * 获取 模块 控制器 方法名
     */
    final static protected function getUrlPath()
    {
        $controller = [];
        ...
        $str = str_replace('App\\Http\\Controllers\\', '',$str);
        $str = str_replace('\\', '/',$str);

        $controller['modules'] = strtolower($str);

        # 控制器名称
        $controller['controller'] = substr(strrchr($class, '\\'), 1);
